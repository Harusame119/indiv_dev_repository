...
public class EXP101Controller {
    ...
    public String register(@Validated @ModelAttribute("formDto") EXP101FormDto formDto,
        ...
        String dispNm = ExConstant.DISPNM_EXP101;
        // 出費登録サービス登録入力
        EXP101ServiceRegisterIn exp101ServiceRegisterIn = new EXP101ServiceRegisterIn();
        ...
        if (!result.hasErrors()) {
            ...
            // 出費登録サービス登録入力設定
            // 金額
            exp101ServiceRegisterIn.setAmount(formDto.getTextAmount());
            // 店舗ID
            exp101ServiceRegisterIn.setStoreId(formDto.getStoreId());
            // 種別ID
            exp101ServiceRegisterIn.setCategoryId(formDto.getCategoryId());
            // 支払者ID
            exp101ServiceRegisterIn.setPayerId(formDto.getPayerId());
            // 支払日
            exp101ServiceRegisterIn.setPaymentDate(formDto.getTextPaymentDate());
            // 分割フラグ
            exp101ServiceRegisterIn.setSplitFlg(formDto.getRadioSplitFlg());
            // 備考
            exp101ServiceRegisterIn.setRemarks(formDto.getTextRemarks());
